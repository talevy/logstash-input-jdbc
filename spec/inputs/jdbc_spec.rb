...
describe "jdbc" do
  ...
  it "should work with watermarks" do
    require "sequel"
    require "sequel/adapters/jdbc"
    Jdbc::Derby.load_driver
    @database = Sequel.connect(mixin_settings['jdbc_connection_string'], :user=> nil, :password=> nil)
    ...
    test_table = @database[:test_table]
    settings = {"statement" => "SELECT num FROM test_table"}
    plugin = LogStash::Inputs::Jdbc.new(mixin_settings.merge(settings))
    plugin.register
    q = Queue.new
    ...
    nums = [10, 20, 30, 40, 50]
    plugin.run(q)
    test_table.insert(:num => nums[0])
    test_table.insert(:num => nums[1])
    plugin.run(q)
    test_table.insert(:num => nums[2])
    test_table.insert(:num => nums[3])
    test_table.insert(:num => nums[4])
    plugin.run(q)
